...
public class GuiLobbyManager : NetworkLobbyManager
{
    ...
    {
        if (lobbyCanvas != null) lobbyCanvas.Hide();
        if (offlineCanvas != null) offlineCanvas.Hide();
        if (onlineCanvas != null) onlineCanvas.Hide();
        if (exitToLobbyCanvas != null) exitToLobbyCanvas.Hide();
        if (connectingCanvas != null) connectingCanvas.Hide();
        if (popupCanvas != null) popupCanvas.Hide();
        if (matchMakerCanvas != null) matchMakerCanvas.Hide();
        if (joinMatchCanvas != null) joinMatchCanvas.Hide();
    }
    public void ShowLobby()
    {
        if (lobbyCanvas != null) lobbyCanvas.Hide();
        if (offlineCanvas != null) offlineCanvas.Show();
        if (onlineCanvas != null) onlineCanvas.Hide();
        if (exitToLobbyCanvas != null) exitToLobbyCanvas.Hide();
        if (connectingCanvas != null) connectingCanvas.Hide();
        if (popupCanvas != null) popupCanvas.Hide();
